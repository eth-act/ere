...
use ere_io_serde::{IoSerde, bincode::BincodeLegacy};
use serde::{Deserialize, Serialize};

/// The basic program takes `BasicProgramInput` as input, and computes
/// `BasicProgramOutput` as output.
pub struct BasicProgram<I = BincodeLegacy>(PhantomData<I>);

impl<I: Default + IoSerde> Program for BasicProgram<I> {
...
mod host {
    ...
    impl<I: Default + IoSerde> BasicProgram<I> {
        pub fn valid_input() -> ProgramTestCase<Self> {
            let mut rng = rng();
        ...
        pub fn invalid_input() -> ProgramTestCase<Self> {
            ProgramTestCase::new(BasicProgramInput {
                should_panic: true,
                ..Default::default()
            })
