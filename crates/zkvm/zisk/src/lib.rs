...
//! | ---------------------------------- | ----- | ------- | ---------------------------------------------------------------------- |
//! | `ZISK_SETUP_ON_INIT`               | Flag  |         | Trigger ROM setup at initialization instead of lazily                  |
//! | `ZISK_PORT`                        | Value |         | Pass `--port {port}` to the server                                     |
//! | `ZISK_UNLOCK_MAPPED_MEMORY`        | Flag  |         | Pass `--unlock-mapped-memory` to the server                            |
//! | `ZISK_MINIMAL_MEMORY`              | Flag  |         | Pass `--minimal_memory` to the server                                  |
//! | `ZISK_PREALLOCATE`                 | Flag  |         | Pass `--preallocate` to the server                                     |
//! | `ZISK_SHARED_TABLES`               | Flag  |         | Pass `--shared-tables` to the server                                   |
//! | `ZISK_MAX_STREAMS`                 | Value |         | Pass `--max-streams {max_streams}` to the server                       |
//! | `ZISK_NUMBER_THREADS_WITNESS`      | Value |         | Pass `--number-threads-witness {number_threads_witness}` to the server |
//! | `ZISK_MAX_WITNESS_STORED`          | Value |         | Pass `--max-witness-stored {max_witness_stored}` to the server         |
//! | `ZISK_START_SERVER_TIMEOUT_SEC`    | Value | `120`   | Timeout waiting for server to start                                    |
//! | `ZISK_SHUTDOWN_SERVER_TIMEOUT_SEC` | Value | `30`    | Timeout for server shutdown                                            |
//! | `ZISK_PROVE_TIMEOUT_SEC`           | Value | `3600`  | Timeout for proof generation                                           |
//!
