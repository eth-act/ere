...
SECTIONS
{
    ...
    /* When unwind tables are disabled, discard .eh_frame to prevent orphan sections.
     * Orphan .eh_frame sections trigger R_RISCV_SET32 relocation overflows at
     * the 0x80000000 base address (signed 32-bit can't represent it).
     * The compiler still emits .eh_frame — we just drop it at link time.
     */
    /DISCARD/ : {
        *(.eh_frame)
        *(.eh_frame.*)
     ...
     * This allows gp-relative addressing to reach ±2KB from this point */
