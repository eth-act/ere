...
pub struct SetupResponse {
    #[prost(oneof = "setup_response::Result", tags = "1, 2")]
    pub result: ::core::option::Option<setup_response::Result>,
}
/// Nested message and enum types in `SetupResponse`.
...
pub trait ZkvmService: Send + Sync {
    async fn setup(
        &self,
        req: twirp::Request<SetupRequest>,
    ) -> twirp::Result<twirp::Response<SetupResponse>>;
    async fn execute(
        &self,
    ...
    T: ZkvmService + Clone + Send + Sync + 'static,
{
    twirp::details::TwirpRouterBuilder::new("/api.ZkvmService", api)
        .route(
            "/Setup",
            |api: T, req: twirp::Request<SetupRequest>| async move {
                api.setup(req).await
            },
        )
        .route(
...
impl ZkvmService for twirp::client::Client {
    async fn setup(
        ...
        self.request("api.ZkvmService/Setup", req).await
    }
    async fn execute(
        &self,
        req: twirp::Request<ExecuteRequest>,
            ...
            match method {
                "Setup" => {
                    twirp::details::encode_response(
                        self
                            .inner
                            .setup(twirp::details::decode_request(req).await?)
                            .await?,
                    )
                }
                "Execute" => {
